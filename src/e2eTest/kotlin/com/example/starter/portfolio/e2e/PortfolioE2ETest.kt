package com.example.starter.portfolio.e2e
...
import com.example.starter.portfolio.grpc.PortfolioServiceGrpc
...
@Import(PortfolioE2ETest.YFinanceStubConfig::class)
class PortfolioE2ETest {
    ...
    @Test
    fun `portfolio optimization across REST and gRPC`() {
        val logger = ScenarioLogger("Portfolio multi-protocol")

        val restResult = webTestClient.post().uri("/api/v1/portfolio/optimize")
        ...
        @Suppress("UNCHECKED_CAST")
        val weights = body["weights"] as Map<String, Number>
        val weightSum = weights.values.sumOf { it.toDouble() }
        expectThat(weightSum).isGreaterThanOrEqualTo(0.99)
        logger.step("REST", "POST /api/v1/portfolio/optimize", "weights=$weights")

        val grpcStub = PortfolioServiceGrpc.newBlockingStub(grpcChannel)
        val grpcResponse = grpcStub.optimize(
            OptimizeRequest.newBuilder()
                .addAllSymbols(listOf("AAPL", "MSFT"))
        ...
        expectThat(grpcResponse.volatility.isFinite()).isTrue()
        logger.step("gRPC", "PortfolioService/Optimize", "volatility=${grpcResponse.volatility}")
